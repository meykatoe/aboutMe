                    ...
                    <main class="flex-1">
                        <section class="grid gap-16 py-16 lg:grid-cols-2 lg:items-center lg:py-28">
                            ...
                            <div class="rounded-2xl border border-black/15 bg-white/60 p-8 shadow-[0px_20px_50px_0px_rgba(0,0,0,0.12)] backdrop-blur-xl dark:border-white/15 dark:bg-white/5 dark:shadow-[0px_20px_50px_0px_rgba(0,0,0,0.5)]">
                                <div class="mx-auto flex max-w-xs flex-col items-center rounded-xl border border-black/10 bg-white/70 p-8 text-center backdrop-blur-md dark:border-white/10 dark:bg-black/40">
                                    ...
                                    <p class="mt-4 text-lg font-semibold text-black dark:text-white">{{ __('你的名字') }}</p>
                                    ...
                                    <div class="mt-6 grid w-full gap-2">
                                        <span class="rounded-lg border border-black/10 bg-white/80 px-4 py-2 text-sm text-black shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5 dark:text-white">{{ __('我的部落格') }}</span>
                                        <span class="rounded-lg border border-black/10 bg-white/80 px-4 py-2 text-sm text-black shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5 dark:text-white">{{ __('作品集') }}</span>
                                        <span class="rounded-lg border border-black/10 bg-white/80 px-4 py-2 text-sm text-black shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5 dark:text-white">{{ __('聯絡我') }}</span>
                                    </div>
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="grid gap-6 py-16 sm:grid-cols-3 lg:py-24">
                            <div class="rounded-xl border border-black/15 bg-white/60 p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] backdrop-blur-xl transition hover:border-black/30 dark:border-white/15 dark:bg-white/5 dark:hover:border-white/30">
                                <h2 class="text-lg font-semibold text-black dark:text-white">{{ __('專屬網址') }}</h2>
                                <p class="mt-3 text-sm leading-relaxed text-black/60 dark:text-white/60">
                                    {!! __('註冊後即可擁有 :url，一個網址分享所有內容。', ['url' => '<code class="rounded border border-black/10 bg-black/5 px-1 py-0.5 dark:border-white/10 dark:bg-white/10">'.e(request()->getHost()).'/'.__('你的名稱').'</code>']) !!}
                                </p>
                            </div>

                            <div class="rounded-xl border border-black/15 bg-white/60 p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] backdrop-blur-xl transition hover:border-black/30 dark:border-white/15 dark:bg-white/5 dark:hover:border-white/30">
                                <h2 class="text-lg font-semibold text-black dark:text-white">{{ __('自動辨識社群圖示') }}</h2>
                                <p class="mt-3 text-sm leading-relaxed text-black/60 dark:text-white/60">
                                    {{ __('貼上 Instagram、GitHub、YouTube 等連結，系統自動顯示對應的品牌圖示。') }}
                                </p>
                            </div>

                            <div class="rounded-xl border border-black/15 bg-white/60 p-6 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] backdrop-blur-xl transition hover:border-black/30 dark:border-white/15 dark:bg-white/5 dark:hover:border-white/30">
                                <h2 class="text-lg font-semibold text-black dark:text-white">{{ __('開源可自架') }}</h2>
                                <p class="mt-3 text-sm leading-relaxed text-black/60 dark:text-white/60">
                                    {{ __('完整原始碼公開，任何人都能部署自己的版本，資料完全掌握在自己手上。') }}
                                </p>
                            </div>
                        </section>
                    </main>
